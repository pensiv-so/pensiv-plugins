...
**Presets** is the second tab. It holds the preset list (add, duplicate, rename, delete, set as default), every convention option on the selected one (columns, name and value alignment, padding character, thousands grouping, list separator, growth arrow, regen wording, bar width), a live preview, and the attribute library.
